...
#        results_file=np.array(results_file)
    ...
    """
    p = Pool(48)
    MAV_models = p.map(cls_mav_dist,range(100))
    p.close()
    p.join()
...
feature_vector_file = pd.read_csv('/home/adhamija/faceness/ImageNet/OpenMax/test_deep_features.txt',
                                  delimiter=' ',header=None)#,memory_map=True)#, nrows=10000)
results_file = pd.read_csv('/home/adhamija/faceness/ImageNet/OpenMax/test_pred.txt',
                           delimiter=' ',header=None)#,memory_map=True)#, nrows=10000)
